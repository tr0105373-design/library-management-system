...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/books", { headers }).then((res) => {
      setBooks(res.data);
      setStats((prev) => ({ ...prev, books: res.data.length }));
    });
    axios.get("http://localhost:5000/api/members", { headers }).then((res) =>
      setStats((prev) => ({ ...prev, members: res.data.length })));
    axios.get("http://localhost:5000/api/issues", { headers }).then((res) => {
      setIssues(res.data);
      setStats((prev) => ({ ...prev, issued: res.data.length }));
    });
    axios.get("http://localhost:5000/api/fines", { headers }).then((res) =>
        ...
        <div style={styles.content}>
          ...
          {overdueBooks.length > 0 && (
            <div style={styles.alertBox}>
              ⚠️ <strong>{overdueBooks.length} Overdue Book(s)!</strong> — Please follow up with members immediately!
            </div>
          )}
          {lowStockBooks.length > 0 && (
            <div style={styles.warningBox}>
              📦 <strong>{lowStockBooks.length} Book(s) Low Stock!</strong> — Consider adding more copies.
            </div>
          )}
          ...
          <div style={styles.chartBoxFull}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={booksData}>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="Total" fill="#4A90D9" radius={[6, 6, 0, 0]} />
                <Bar dataKey="Available" fill="#5BAD72" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div style={styles.chartBoxFull}>
            ...
            {recentIssues.length === 0 ? (
              <p style={{ color: "#999", textAlign: "center", padding: "20px" }}>No issues yet!</p>
            ) : (
              <table style={styles.table}>
                <thead>
                  <tr style={styles.thead}>
              ...
              <table style={styles.table}>
                ...
                <tbody>
                  {lowStockBooks.map((b, idx) => (
                    <tr key={b.book_id} style={{ backgroundColor: idx % 2 === 0 ? "#fffdf0" : "white" }}>
                      <td style={styles.td}>{b.book_id}</td>
                      <td style={styles.td}><strong>{b.title}</strong></td>
                      <td style={styles.td}>{b.author}</td>
                      <td style={{ ...styles.td, color: "#F0A500", fontWeight: "bold" }}>{b.available_copies}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
