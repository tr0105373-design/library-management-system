...
function Fines() {
  ...
  const handlePay = async (e) => {
    e.preventDefault();
    try { await axios.post(`${API_URL}/api/fines/pay`, payForm, { headers }); setMessage("✅ Fine paid!"); fetchFines(); setPayForm({ fine_id: "", paid_amount: "" }); }
    catch { setMessage("❌ Error paying fine!"); }
  };

  const handleWaive = async (e) => {
    e.preventDefault();
    try { await axios.post(`${API_URL}/api/fines/waive`, waiveForm, { headers }); setMessage("✅ Fine waived!"); fetchFines(); setWaiveForm({ fine_id: "", reason: "" }); }
      ...
      <div style={styles.layout}>
        ...
        <div style={styles.content}>
          ...
          <div style={styles.cards}>
            <div style={{ ...styles.card, borderLeft: "5px solid #E07B54" }}><h2 style={{ color: "#E07B54", margin: 0 }}>{pendingFines.length}</h2><p style={styles.cardLabel}>⚠️ Pending</p></div>
            <div style={{ ...styles.card, borderLeft: "5px solid #F0A500" }}><h2 style={{ color: "#F0A500", margin: 0 }}>Rs. {totalPending}</h2><p style={styles.cardLabel}>💸 Total Pending</p></div>
            <div style={{ ...styles.card, borderLeft: "5px solid #5BAD72" }}><h2 style={{ color: "#5BAD72", margin: 0 }}>{paidFines.length}</h2><p style={styles.cardLabel}>✅ Paid</p></div>
            <div style={{ ...styles.card, borderLeft: "5px solid #4A90D9" }}><h2 style={{ color: "#4A90D9", margin: 0 }}>Rs. {totalCollected}</h2><p style={styles.cardLabel}>💰 Collected</p></div>
            <div style={{ ...styles.card, borderLeft: "5px solid #9B59B6" }}><h2 style={{ color: "#9B59B6", margin: 0 }}>{waivedFines.length}</h2><p style={styles.cardLabel}>🔓 Waived</p></div>
          </div>
          ...
          <div style={{ display: "flex", gap: "20px", marginBottom: "20px", flexWrap: "wrap" }}>
            <div style={{ ...styles.formBox, flex: 1 }}>
              ...
              <form onSubmit={handlePay} style={{ display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "flex-end" }}>
                <div style={styles.formGroup}><label style={styles.label}>Fine ID *</label><input style={styles.input} placeholder="Fine ID" value={payForm.fine_id} onChange={(e) => setPayForm({ ...payForm, fine_id: e.target.value })} required /></div>
                <div style={styles.formGroup}><label style={styles.label}>Amount *</label><input style={styles.input} placeholder="Amount" type="number" value={payForm.paid_amount} onChange={(e) => setPayForm({ ...payForm, paid_amount: e.target.value })} required /></div>
            ...
            <div style={{ ...styles.formBox, flex: 1 }}>
              ...
              <form onSubmit={handleWaive} style={{ display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "flex-end" }}>
                <div style={styles.formGroup}><label style={styles.label}>Fine ID *</label><input style={styles.input} placeholder="Fine ID" value={waiveForm.fine_id} onChange={(e) => setWaiveForm({ ...waiveForm, fine_id: e.target.value })} required /></div>
                <div style={styles.formGroup}><label style={styles.label}>Reason *</label><input style={styles.input} placeholder="Reason" value={waiveForm.reason} onChange={(e) => setWaiveForm({ ...waiveForm, reason: e.target.value })} required /></div>
                <button style={styles.waiveBtn} type="submit">🔓 Waive</button>
              </form>
            </div>
          </div>

          {[{ title: "⚠️ Pending Fines", data: pendingFines, bg: "#fff9f5" }, { title: "✅ Paid Fines", data: paidFines, bg: "#f5fff8" }].map(({ title, data, bg }) => (
            <div key={title} style={styles.tableBox}>
              <h4 style={styles.tableTitle}>{title} ({data.length})</h4>
              {data.length === 0 ? <p style={{ color: "#999", textAlign: "center", padding: "20px" }}>No records!</p> : (
                <table style={styles.table}>
                  <thead><tr style={styles.thead}><th style={styles.th}>Fine ID</th><th style={styles.th}>Member</th><th style={styles.th}>Amount</th><th style={styles.th}>Status</th></tr></thead>
                  <tbody>{data.map((f, i) => (
                    <tr key={f.fine_id} style={{ backgroundColor: i % 2 === 0 ? bg : "white" }}>
                      <td style={styles.td}>{f.fine_id}</td><td style={styles.td}>{f.name}</td>
                      <td style={styles.td}>Rs. {f.status === "paid" ? f.paid_amount : f.amount}</td>
                      <td style={styles.td}><span style={{ padding: "3px 10px", borderRadius: "12px", fontSize: "12px", backgroundColor: f.status === "paid" ? "#d4edda" : "#fff3cd", color: f.status === "paid" ? "#155724" : "#856404" }}>{f.status}</span></td>
                    </tr>
                  ))}</tbody>
                </table>
              )}
